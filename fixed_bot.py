...
def E(emoji: str) -> str:
    """Return a premium tg-emoji tag if available, else the plain emoji."""
...
class CardCheckerBot:
    def __init__(self):
        ...
        # New locks and state for improved resilience
        self._site_test_lock = asyncio.Lock()
        self._price_cache_lock = asyncio.Lock()
        self._captcha_strike_counts: Dict[str, int] = {}
        self._permacaptcha_dead: Set[str] = set()
    ...
    async def mark_site_dead(self, site: str, reason: str = "", captcha: bool = False):
        """Mark a site as dead. If captcha=True, track strikes; on 2nd strike → permanent ban."""
        async with self._site_lock:
            ...
            if captcha:
                ...
                strikes = self._captcha_strike_counts.get(site, 0) + 1
                self._captcha_strike_counts[site] = strikes
                if strikes >= 2:
                    # Permanent CAPTCHA ban
                    self._permacaptcha_dead.add(site)
                    logger.warning(f"🚫 PERMANENT CAPTCHA BAN: {site} (strikes={strikes})")
                else:
                    # Temporary block
    ...
    async def _unblock_captcha_sites(self):
        """Re-add CAPTCHA-blocked sites whose cooldown has expired. Skip permanently banned sites."""
        now = time.time()
        unblocked = []
        async with self._site_lock:
            for site, unblock_time in list(self._captcha_blocked_sites.items()):
                if now >= unblock_time:
                    ...
                    # Skip permanently banned sites
                    if site in self._permacaptcha_dead:
                        continue
        ...
        try:
            ...
            async with session.get(SHOPIFY_API_URL, params=params, timeout=timeout) as resp:
                ...
                if 'application/json' not in content_type:
                    ...
                    logger.warning(f"[API] {site_name} | Non-JSON response: {content_type} | {raw_text[:120]}")
                    # Track consecutive API errors
                    self._consecutive_api_errors += 1
                    if self._consecutive_api_errors >= 5:
                        self._api_unavailable = True
                        logger.error(f"[API] ⚠️ API marked UNAVAILABLE after {self._consecutive_api_errors} consecutive errors")
                    return ShopifyCheckResult(
                        card=card_line, status=ShopifyCheckStatus.ERROR,
                        status_code="API_NON_JSON", site_name=site_name,
                        shop_url=shop_url, gateway="SHOPIFY-RELOADED",
                        error_msg=f"Non-JSON response: {content_type[:60]}",
                        retryable=True, site_dead=False,
                    )
                try:
                    data = await resp.json()
                except Exception as json_err:
                    logger.warning(f"[API] {site_name} | JSON parse error: {json_err}")
                    self._consecutive_api_errors += 1
                    if self._consecutive_api_errors >= 5:
                        self._api_unavailable = True
                    return ShopifyCheckResult(
                        card=card_line, status=ShopifyCheckStatus.ERROR,
                        status_code="API_JSON_ERROR", site_name=site_name,
                        shop_url=shop_url, gateway="SHOPIFY-RELOADED",
                        error_msg=f"JSON parse error: {str(json_err)[:80]}",
                        retryable=True, site_dead=False,
                    )
                ...
                if self._api_unavailable:
                    ...
                    logger.info("[API] ✅ API back online — resuming normal operation")
                elapsed = time.time() - start_time
    ...
    async def shopify_check_card(self, card_line: str, proxy_str: Optional[str] = None,
        ...
        # Pause if API is unavailable
        if self._api_unavailable:
            logger.warning("[shopify] API unavailable — pausing 60s before retry")
    ...
    async def _test_site_via_api(self, shop_url: str) -> dict:
        ...
        Retries up to 2 times on non-200 HTTP status before marking dead.
        Transient errors (TAX, DELIVERY, PAYMENTS) are treated as GOOD — they prove the store processes payments."""
        start_time = time.time()
        site_name = shop_url.replace("https://", "").replace("http://", "")
        ...
        for attempt in range(SITE_TEST_RETRIES + 1):
            try:
                ...
                async with session.get(SHOPIFY_API_URL, params=params, timeout=timeout) as resp:
                    ...
                    if resp.status != 200:
                        logger.warning(f"[site-test] {site_name} | HTTP {resp.status} | attempt {attempt+1}/{SITE_TEST_RETRIES+1}")
                        if attempt < SITE_TEST_RETRIES:
                            await asyncio.sleep(2)
                            continue
                        out["reason"] = f"HTTP {resp.status} after {elapsed:.1f}s (all retries failed)"
                        logger.info(f"[site-test] {site_name} → DEAD (HTTP {resp.status} after {SITE_TEST_RETRIES} retries) | {elapsed:.1f}s")
                        return out
                    ...
                    if 'application/json' not in content_type:
                        raw_text = await resp.text()
                        logger.warning(f"[site-test] {site_name} | non-JSON response: {content_type} | {raw_text[:80]}")
                        if attempt < SITE_TEST_RETRIES:
                            await asyncio.sleep(2)
                            continue
                        out["reason"] = f"Non-JSON response: {content_type}"
                        return out

                    try:
                        data = await resp.json()
                    except Exception as json_err:
                        logger.warning(f"[site-test] {site_name} | JSON parse error: {json_err}")
                        if attempt < SITE_TEST_RETRIES:
                            await asyncio.sleep(2)
                            continue
                        out["reason"] = f"JSON parse error: {json_err}"
                        return out
                    ...
                    # CAPTCHA detection — track strikes
                    if "CAPTCHA" in response_upper:
                        out["captcha"] = True
                        out["reason"] = f"CAPTCHA ({api_response})"
                        logger.info(f"[site-test] {site_name} → CAPTCHA at {elapsed:.1f}s")
                        return out

                    # Dead site markers — definitive dead
                    dead_markers = [
                        "NOT_A_SHOPIFY", "STORE_CLOSED", "PASSWORD_PROTECTED",
                        "NO_PRODUCTS", "STORE_NOT_FOUND", "404",
                        "PAYMENTS_PAYMENT_FLEXIBILITY_TERMS_ID_MISMATCH",
                    ...
                    for marker in dead_markers:
                        if marker in response_upper:
                            ...
                            logger.info(f"[site-test] {site_name} → DEAD ({marker}) | {elapsed:.1f}s")
                            return out

                    # "Store temporarily unavailable" string check
                    if "store temporarily unavailable" in api_response.lower():
                        out["reason"] = f"Dead: Store temporarily unavailable"
                        logger.info(f"[site-test] {site_name} → DEAD (store temporarily unavailable) | {elapsed:.1f}s")
                        return out

                    # BAD site markers: generic errors that tell us nothing about payment
                    bad_markers = [
                    ...
                    for marker in bad_markers:
                        if marker in response_upper:
                            ...
                            out["bad_site"] = True
                            logger.info(f"[site-test] {site_name} → BAD ({marker}) | {elapsed:.1f}s")
                            return out

                    # GOOD markers: transient errors prove the store is processing payments!
                    # These are NOT dead — they confirm the store's payment flow is alive.
                    good_markers = [
                    ...
                    if any(m in response_upper for m in good_markers):
                        ...
                        logger.info(f"[site-test] {site_name} → ✅ GOOD ({api_response}) | {elapsed:.1f}s")
                        return out

                    # Broader working markers: DELIVERY_ or PAYMENTS_ prefix = store is alive
                    if any(kw in response_upper for kw in ["DELIVERY_", "PAYMENTS_"]):
                        out["working"] = True
                        out["good"] = True
                        out["reason"] = f"OK (transient): {api_response} via {api_gateway}"
                        out["price"] = data.get("Price", 0.0)
                        logger.info(f"[site-test] {site_name} → ✅ WORKING (transient: {api_response}) | {elapsed:.1f}s")
                        return out

                    # DECLINED (no prefix) and broad working markers
                    working_markers = ["DECLINED"]
                    if any(m in response_upper for m in working_markers):
                        out["working"] = True
                        out["reason"] = f"OK: {api_response} via {api_gateway}"
                        out["price"] = data.get("Price", 0.0)
                        logger.info(f"[site-test] {site_name} → ✅ WORKING (DECLINED) | {elapsed:.1f}s")
                        return out
                    ...
                    if api_status:
                        ...
                        logger.info(f"[site-test] {site_name} → ✅ WORKING (Status=True) | {elapsed:.1f}s")
                        return out

                    # Unknown — retry once more
                    if attempt < SITE_TEST_RETRIES:
                        await asyncio.sleep(2)
                        continue

                    out["reason"] = f"Unknown response: {api_response}"
                    logger.info(f"[site-test] {site_name} → DEAD (unknown: {api_response}) | {elapsed:.1f}s")
                    return out

            except asyncio.TimeoutError:
                elapsed = time.time() - start_time
                logger.warning(f"[site-test] {site_name} | TIMEOUT attempt {attempt+1} | {elapsed:.1f}s")
                if attempt < SITE_TEST_RETRIES:
                    await asyncio.sleep(2)
                    continue
                ...
                logger.warning(f"[site-test] {site_name} | Connection error attempt {attempt+1}: {str(e)[:60]}")
                if attempt < SITE_TEST_RETRIES:
                    await asyncio.sleep(2)
                    continue
                ...
                logger.warning(f"[site-test] {site_name} | Error attempt {attempt+1}: {type(e).__name__}: {str(e)[:60]}")
                if attempt < SITE_TEST_RETRIES:
                    await asyncio.sleep(2)
                    continue
                ...
                for idx, card in enumerate(cards, 1):
                    ...
                    try:
                        if gateway in ['stripe', 'braintree']:
                            ...
                            # Pause if API is globally unavailable
                            if self._api_unavailable:
                            ...
                            if ok:
                                ...
                                if is_retryable and retry_count < MAX_CARD_RETRIES:
                                    ...
                                    backoff_delay = 2 ** (retry_count + 1)  # 2, 4, 8
                                    logger.info(
                                        f"[worker] ♻️ Re-queued card {card[:6]}... for retry "
                                        f"{retry_count + 1}/{MAX_CARD_RETRIES} | backoff={backoff_delay}s | reason={reason}"
                                    )
    ...
    async def _api_health_check(self) -> bool:
        """Check if the Shopify API is reachable. Returns True if healthy."""
        test_card = "4111111111111111|01|26|123"
        try:
            session = await self.get_http_session()
            params = {"cc": test_card, "site": "https://yallsweettea.com"}
            timeout = aiohttp.ClientTimeout(total=10)
            async with session.get(SHOPIFY_API_URL, params=params, timeout=timeout) as resp:
                if resp.status == 200:
                    logger.info("✅ [health] API health check PASSED")
                    self._api_health_fail_count = 0
                    self._api_healthy = True
                    return True
                else:
                    logger.warning(f"⚠️ [health] API health check FAILED: HTTP {resp.status}")
                    self._api_health_fail_count += 1
                    return False
        except Exception as e:
            logger.warning(f"⚠️ [health] API health check ERROR: {e}")
            self._api_health_fail_count += 1
            return False

    async def _background_health_ping(self):
        """Run API health ping every 5 minutes. Pause all mass workers on 3 consecutive failures."""
        while True:
            await asyncio.sleep(300)  # 5 minutes
            ok = await self._api_health_check()
            if not ok:
                if self._api_health_fail_count >= 3:
                    logger.error(f"❌ [health] API failed {self._api_health_fail_count} times — marking unavailable")
                    self._api_healthy = False
                    self._api_unavailable = True
            else:
                if not self._api_healthy:
                    logger.info("✅ [health] API recovered — resuming mass workers")
                self._api_healthy = True
                self._api_unavailable = False
                self._api_health_fail_count = 0
            ...
            try:
                ...
                if os.path.exists("checker_session.session"):
                    try:
                        ...
                        logger.warning(f"⚠️ user_client (Stripe/Braintree) failed to start: {uc_err} — continuing without it")
                        self.user_client = None
                else:
                    logger.warning("⚠️ checker_session.session not found — Stripe/Braintree disabled. Shopify still works.")
                    self.user_client = None
                ...
                health_task = asyncio.create_task(self._background_health_ping())
                await asyncio.gather(bot_task, health_task, *self.worker_tasks)
    ...
    async def run(self):
        ...
        if self.owner_sites:
            ...
            # Price prefetch disabled — avoids API flood at startup
            # (prices are fetched on demand via site testing)
        # API health check on startup — non-blocking: log warning and continue if it fails
        logger.info("🔍 Performing API health check before starting...")
        if await self._api_health_check():
            logger.info("✅ API health check passed")
        else:
            logger.warning("⚠️ API health check failed at startup — starting anyway (will retry in background)")
