...
class CardCheckerBot:
    ...
    async def _api_health_check(self) -> bool:
        ...
        try:
            session = await self.get_http_session()
            params = {"cc": test_card, "site": "https://example.myshopify.com"}
    ...
    async def run(self):
        ...
        for attempt in range(3):
            if await self._api_health_check():
                health_ok = True
                break
            logger.warning(f"⚠️ API health check failed (attempt {attempt+1}/3), retrying in 30s...")
            await asyncio.sleep(30)
        if not health_ok:
            logger.warning("⚠️ API health check could not confirm API is up — starting anyway (will retry during operation)")
