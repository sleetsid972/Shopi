...
def parse_proxy(proxy_str):
    ...
    if len(parts) == 2:
        ...
        # FIX: URL-encode credentials to handle special characters
...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        connector = aiohttp.TCPConnector(ssl=False, limit=50, limit_per_host=10, ttl_dns_cache=300, force_close=False, enable_cleanup_closed=True)  # FIX: Added limits for VPS
        timeout = aiohttp.ClientTimeout(total=15)  # FIX: Increased from 10s for VPS
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        connector = aiohttp.TCPConnector(ssl=False, limit=50, limit_per_host=10, ttl_dns_cache=300, force_close=False, enable_cleanup_closed=True)  # FIX: Added limits for VPS
        timeout = aiohttp.ClientTimeout(total=45)  # FIX: Increased from 30s for VPS reliability
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            for i in range(2):
                ...
                if i == 0:
                    await asyncio.sleep(1)  # FIX: Reduced from 3s
                ...
                try:
                    ...
                    if receipt_data:
                        ...
                        if receipt_data.get('__typename') in ['ProcessingReceipt', 'WaitingReceipt']:
                            await asyncio.sleep(2)  # FIX: Reduced from 4s
                            continue
