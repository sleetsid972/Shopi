...
║  💀 PREMIUM EDITION V5 — SAVAGE UI + blury ENGINE   ║
╚══════════════════════════════════════════════════════╝
"""
...
# ======================== CONFIGURATION ========================
BOT_TOKEN = "8044466733:AAFUc-fP8fvFMOwnEhwW05airbLk3krraQw"
...
class CardCheckerBot:
    def __init__(self):
        ...
        # STABLE: Global semaphore to limit concurrent Shopify API calls (prevents 429/402)
            ...
            async with session.get(SHOPIFY_API_URL, params=params, timeout=timeout) as resp:
                ...
                if resp.status != 200:
                    ...
                    return ShopifyCheckResult(
                        card=card_line, status=ShopifyCheckStatus.DECLINED if is_402 else ShopifyCheckStatus.ERROR,
                        ...
                        retryable=False if is_402 else resp.status >= 500,
                        site_dead=resp.status == 404,
                    )
                            ...
                            async with results_lock:
                                ...
                                if result.get("good"):
                                    ...
                                    if price and price > 0:
                                        ...
                                        # Fallback: scrape product price if API didn't return one
    ...
    def load_working_sites_from_file(self):
        ...
        Also loads good_sites_api.txt and site_prices.json if available."""
        # Load good sites
    ...
    async def format_shopify_result(self, card_line: str, approved: bool, info: dict,
        ...
        if approved:
            ...
            return (
                ...
                "╠══════════════════════════════════════╣\n"
                f"║  💎 Shopify Reloaded ENTITY X BLURY FACE V2 ENGINE       ║\n"
                "╚══════════════════════════════════════╝\n\n"
                "┌──────── 💳 𝗖𝗮𝗿𝗱 𝗗𝗲𝘁𝗮𝗶𝗹𝘀 ─────────┐\n"
                "│\n"
                f"│  💳 <code>{cc}|{mm}|{yy}|{cvv}</code>\n"
            ...
            return (
                ...
                "║  🛒 𝗦𝗛𝗢𝗣𝗜𝗙𝗬  ─  𝗗𝗘𝗖𝗟𝗜𝗡𝗘𝗗 ❌💀  ║\n"
                "╠══════════════════════════════════════╣\n"
                "║  💎 Shopify Reloaded ENTITY X BLURY FACE V2 ENGINE       ║\n"
                "╚══════════════════════════════════════╝\n\n"
                "┌──────── 💳 𝗖𝗮𝗿𝗱 𝗗𝗲𝘁𝗮𝗶𝗹𝘀 ─────────┐\n"
                "│\n"
                f"│  💳 <code>{cc}|{mm}|{yy}|{cvv}</code>\n"
    ...
    async def start_bot(self):
        ...
        async def callback(event):
            ...
            # ━━━━━━ ADMIN PANEL ━━━━━━
                ...
                try:
                    ...
                    # OPTIMISED: prefetch prices after site testing
                    try:
                        await self.prefetch_site_prices()
                    except Exception as pe:
                        logger.warning(f"⚠️ Price prefetch after test_sites (button) failed: {pe}")
        ...
        async def test_sites_cmd(event):
            ...
            try:
                ...
                # OPTIMISED: prefetch prices for all working sites after testing
                try:
                    await self.prefetch_site_prices()
                except Exception as pe:
                    logger.warning(f"⚠️ Price prefetch after test_sites failed: {pe}")
                ...
                await status_msg.edit(
                    ...
                    f"❌ Dead: <code>{len(dead)}</code>\n"
                    f"📦 Loaded: <code>{loaded}</code> sites activated\n"
                    f"💰 Prices cached: <code>{cached_prices}/{loaded}</code>\n\n"
        ...
        # ━━━━━━ /refresh_prices — Re-fetch prices for all working sites (admin only) ━━━━━━
        @self.bot_client.on(events.NewMessage(pattern=r'/refresh_prices'))
        async def refresh_prices_cmd(event):
            if event.sender_id not in ADMINS:
                return
            sites = list(self.good_sites) if self.good_sites else list(self.working_sites)
            if not sites:
                await event.reply("❌ No working sites. Run /test_sites first.")
                return
            status_msg = await event.reply(
                f"🔄 Refreshing prices for {len(sites)} sites...\n"
                "⏳ This fetches prices without re-testing sites.",
                parse_mode='html'
            )
            try:
                ...
                await status_msg.edit(f"❌ Error: <code>{str(e)[:100]}</code>", parse_mode='html')
        ...
        async def file_handler(event):
            ...
            if any(job['user_id'] == uid and not job.get('stop') for job in self.active_jobs.values()):
            ...
            if gateway == 'shopify' and total_cards > 1:
                ...
                msg = await event.reply(
                    ...
                    f"┃ 🔀 Workers: <code>{num_chunks}</code> parallel chunks\n"
                    f"┃ ⚡ Max throughput: <code>~{num_chunks} cards/parallel</code>",
    ...
    async def update_aggregated_progress(self, chat_id: int, msg_id: int, user_id: int):
        ...
        try:
            ...
            if processed > 0 and elapsed_secs > 0:
                cards_per_sec = processed / elapsed_secs
                cards_per_min = cards_per_sec * 60
            ...
            num_workers = self.user_job_count.get(user_id, 1)
                ...
                for idx, card in enumerate(cards, 1):
                    ...
                    try:
                        if gateway in ['stripe', 'braintree']:
                            ...
                            # OPTIMISED: no asyncio.wait_for wrapper — API already has its own timeout
                            try:
                    ...
                    # PARALLEL: update aggregated processed count and shared progress message
                    if is_parallel:
                        async with self._aggregated_lock:
                            agg = self.user_aggregated.get(user_id)
                            if agg:
                                agg['processed'] += 1
                        # STABLE: Update shared progress message every 20 cards to avoid Telegram flood
                        if processed % 20 == 0 or processed == total:
                            msg_id = self._user_progress_msg.get(user_id)
                            if msg_id:
                                await self.update_aggregated_progress(chat_id, msg_id, user_id)
                    else:
                        # Non-parallel: update per-job progress every 20 cards to avoid Telegram flood
                        if processed % 20 == 0 or processed == total:
                            elapsed = datetime.now() - start_time
                            elapsed_secs = elapsed.total_seconds()
                            if processed > 0 and elapsed_secs > 0:
                    ...
                    if gateway != 'shopify':
                        ...
                        # STABLE: small delay between Shopify cards to smooth out bursts and avoid 429
